$.fn.quietflow = function(attributes) {
    ...
    var theme = "starfield";
    var z_Index = -1000;
    if (attributes != undefined) { if (attributes.z_Index != undefined) { z_Index = attributes.z_Index; } }
    var backgroundCol, speed, lineGlow, specificColors = [],
        transparent = true,
        squareSize = 10,
        maxRed = 255,
        maxGreen = 255,
    ...
    function styledMessage(message, background, color) { console.log("%c " + message, "background: " + background + "; color: " + color); }
    if (attributes != undefined) {
        ...
        } else if (attributes.theme == "simpleGradient") { theme = "simpleGradient"; if (attributes.primary != undefined) primary = attributes.primary; if (attributes.accent != undefined) accent = attributes.accent; } else if (attributes.theme == "starfield") {
            theme = "starfield";
    ...
    } else { styledMessage("You need to specify some attributes!", "#D6D6D6", "#DF0000"); }
    ...
    function init() {
        if (theme == "squareFlash") {
            ...
            function vortexTimed() {
                if (x + dx > $limitX || x + dx < 0) { dx = -dx; }
                if (y + dy > $limitY || y + dy < 0) { dy = -dy; }
                x += dx;
                y += dy;
            ...
            circleData = [];
            for (var i = 0; i < bounceBallCount; i++) { if (specificColors.length == 0) { if (transparent) { circleData.push([Math.random() * $limitX, Math.random() * $limitY, Math.random() * maxRadius, Math.random() * 2, Math.random() * 4, randCol(255, 255, 255, .5)]); } else { circleData.push([Math.random() * $limitX, Math.random() * $limitY, Math.random() * maxRadius, Math.random() * 2, Math.random() * 4, randCol(255, 255, 255)]); } } else { circleData.push([Math.random() * $limitX, Math.random() * $limitY, Math.random() * maxRadius, Math.random() * 2, Math.random() * 4, specificColors[Math.floor(Math.random() * specificColors.length)]]); } }
            ...
            function bouncingBallsTimed() {
                ...
                for (var i = 0; i < bounceBallCount; i++) {
                    ...
                    var X = 0,
                        ...
                        DY = 4,
                        COLOR = 5;
                    if (current[X] + current[DX] > $limitX || current[X] + current[DX] < 0) { current[3] = -current[3]; }
                    if (current[Y] + current[DY] > $limitY || current[Y] + current[DY] < 0) { current[DY] = -current[DY]; }
            ...
            var starData = [];
            for (var i = 0; i < 200; i++) { starData.push([Math.random() * $limitX * 2 - $limitX, Math.random() * $limitY, Math.random() * starSize, Math.ceil(Math.random() * 5)]); }
            setTimeout(starfieldTimed, speed);

            function starfieldTimed() {
                var gradient = ctx.createLinearGradient(0, 0, $limitX / 2, $limitY);
                gradient.addColorStop(0, "#417bff");
                gradient.addColorStop(1, "#417bff");
                ctx.fillStyle = gradient;
                ctx.fillRect(0, 0, $limitX, $limitY);
            ...
            for (var i = 0; i < triangles; i++) {
                ...
                ctx.lineTo(Math.random() * $limitX, Math.random() * $limitY);
                ctx.lineTo(Math.random() * $limitX, Math.random() * $limitY);
                if (specificColors.length > 0) { ctx.fillStyle = specificColors[Math.floor(Math.random() * specificColors.length)]; } else { ctx.fillStyle = randCol(255, 255, 255, .5); }
                ctx.closePath();
                ctx.fill();
            }
            ...
            var squareData = [];
            for (var i = 0; i < boxCount; i++) { if (specificColors.length == 0) { if (transparent) { squareData.push([Math.random() * $limitX * 2 - $limitX, Math.random() * $limitY, Math.random() * maxBoxSize + 1, randCol(255, 255, 255, .5), Math.random() * 5]); } else { squareData.push([Math.random() * $limitX * 2 - $limitX, Math.random() * $limitY, Math.random() * maxBoxSize + 1, randCol(255, 255, 255), Math.random() * 5]); } } else { squareData.push([Math.random() * $limitX * 2 - $limitX, Math.random() * $limitY, Math.random() * maxBoxSize + 1, specificColors[Math.floor(Math.random() * specificColors.length)], Math.random() * 5]); } }
            ...
            function floatingBoxesTimed() {
                ...
                for (var i = 0; i < squareData.length; i++) {
                    ...
                    if (current[X] > $limitX + maxBoxSize || current[Y] < -maxBoxSize) { squareData.splice(i, 1); if (specificColors.length == 0) { if (transparent) { squareData.push([Math.random() * $limitX * 2 - $limitX, Math.random() * $limitY * 2 + $limitY, Math.random() * maxBoxSize + 1, randCol(255, 255, 255, .5), Math.random() * 5]); } else { squareData.push([Math.random() * $limitX * 2 - $limitX, Math.random() * $limitY * 2 + $limitY, Math.random() * maxBoxSize + 1, randCol(255, 255, 255), Math.random() * 5]); } } else { squareData.push([Math.random() * $limitX * 2 - $limitX, Math.random() * $limitY * 2 + $limitY, Math.random() * maxBoxSize + 1, specificColors[Math.floor(Math.random() * specificColors.length)], Math.random() * 5]); } }
                }
                setTimeout(floatingBoxesTimed, 100);
            }
        }
